# Tutorial 02
# Julia version 1.6.1
# Packages versions on Manifeste.toml
...
scene, layout = layoutscene(resolution=(650,650))
scene
...
# add Legend
leg = Legend(scene, [lineobject1,lineobject2], ["Supply", "Demand"], halign = :right,
    valign = :top, tellheight = false, tellwidth = false, margin = (10,10,10,10),
    labelsize = 7, linewidth = 1, padding = (10,10,5,5), patchsize = (20,10)
)
layout[1,1] = leg
scene
...
e_Q, e_P = A \ b
println("Equlibrium Price:\t",e_P)
println("Equlibrium Quantity:\t",e_Q)

# plot equilibrium price an quantitu
...
# save plot
save("tutorial_02/supply-demand.svg",scene)
...
r = 3.92 / 100 /12  # convert % to decimal then convert to annual to monthly
...
# calculate total cost of Mortage

total = c * N 

# mortgage table function
function pmt_table(APR, years, amouunt)
    # initialize variables
    r = APR / 100 / 12
    N = years * 12
    P = amouunt
...
writedlm("tutorial_02/pmt_table.csv",data,',')
...
leg1 = Legend(scene, [lineobject1,lineobject2,lineobject3], ["Principal", "Interest","Total Payment"], halign = :right,
    valign = :center, tellheight = false, tellwidth = false, margin = (10,10,10,10),
    labelsize = 5, linewidth = 1, padding = (10,10,5,5), patchsize = (15,5)
)
layout[1,1] = leg1
scene
...
leg2 = Legend(scene, [lineobject1,lineobject2], ["Principal", "Interest"], halign = :center,
    valign = :top, tellheight = false, tellwidth = false, margin = (10,10,10,10),
    labelsize = 5, linewidth = 1, padding = (10,10,5,5), patchsize = (15,5)
)
layout[1,2] = leg2
...
# save plot
save("tutorial_02/mortage-calculator.svg",scene)