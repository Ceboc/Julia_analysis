...
savehtml("tutorial_08/mydf_url",mydf_url)
CSV.write("tutorial_08/mydf_url.csv",mydf_url)

# download 2019 gdp data from GitHub
...
savehtml("tutorial_08/df_gdp",df_gdp)
CSV.write("tutorial_08/df_gdp.csv",df_gdp)
...
# DataFramesMeta
# Southern Asia Subregion countries using filter() from JuliaBase

df_sa_base = filter(:subregion => x -> x == "Southern Asia", df_inner)
savehtml("tutorial_08/df_sa_base",df_sa_base)

using DataFramesMeta

# Southern Asia Subregion countries using @where from DataFramesMeta
df_sa_dfm = @where(df_inner,:subregion .== "Southern Asia")
savehtml("tutorial_08/df_sa_dfm",df_sa_dfm)

# multiple filters using @where macro
df_multi_filters = @where(df_inner,
    .|(:subregion .== "Southern Asia", :subregion .== "Northern America"), # or condition
    .&(:my19 .< 1, :gdppc .> 10)    # and condition
)
savehtml("tutorial_08/df_multi_filters",df_multi_filters)
...
savehtml("tutorial_08/df_by_r",df_by_r)

# copy and pasete code to summarize data by subregion
...
savehtml("tutorial_08/df_by_s",df_by_s)
...
# how to save plot
img = SVG("tutorial_08/p_bar_region.svg", 16cm, 9cm)
draw(img,p)
...
img = SVG("tutorial_08/p_bar_subregion.svg", 16cm, 9cm)
draw(img,p)
...
img = SVG("tutorial_08/p_point_country.svg", 16cm, 9cm)
draw(img,p)
...
img = SVG("tutorial_08/p_beeswarm_region.svg", 16cm, 9cm)
draw(img,p)

# 5. boxplot plot by region
...
img = SVG("tutorial_08/p_boxplot_region.svg", 16cm, 9cm)
draw(img,p)


# 6. density plt by region
...
p = plot(df_sub100_o,
    ...
    Guide.title("2019 Population Density by Regon (sub 100 mil ex Oceania)"),
    Guide.colorkey(title="Region"),
    Scale.x_continuous(format=:plain),
    Theme(background_color="white")
)

img = SVG("tutorial_08/p_density_region.svg", 16cm, 9cm)
draw(img,p)
...
img = SVG("tutorial_08/p_violin_region.svg", 16cm, 9cm)
draw(img,p)